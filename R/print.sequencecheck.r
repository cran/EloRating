print.sequencecheck <- function(x, ...) {
  ...
  if(x$checksum["selfinteractions"]+x$checksum["IDcheck"]+x$checksum["length"]==0) cat("Everything seems to be fine with the interaction sequence...OK\n")
  ...
  if(sum(x$checksum[c("startpresence1", "startpresence2", "endpresence1", "endpresence2", "IDmatch", "IA_presencematch", "presenceentries", "datecol")])==0) cat("presence data seems to be fine and matches interaction sequence...OK\n")
